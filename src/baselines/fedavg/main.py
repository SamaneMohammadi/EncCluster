...
os.environ['DATA_DIR'] = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),'datasets')
...
parser.add_argument("--model",					default='convnext',			type=str,		help="model to be used")
...
parser.add_argument("--device",					default='cuda:0',			type=str,		help="device to utilize")
...
def client_fn(cid):
	...
	return Client(int(cid), num_clients=args.num_clients, model_loader=Model, model_name=args.model,
		data_loader=load_data, device=args.device, batch_size=args.batch_size, data_name=args.dataset,
		use_cutmix=args.use_cutmix, skewness=args.skewness, input_res=args.input_res, seed=args.seed)
...
def server_fn():
	...
	return Server(num_rounds=args.num_rounds, num_clients=args.num_clients, participation=args.participation,
        model_loader=Model, model_name=args.model, data_loader=load_data, batch_size=args.batch_size, model_fp=logger_name,
		data_name=args.dataset, num_epochs=args.num_epochs, input_res=args.input_res, lr=args.lr)
