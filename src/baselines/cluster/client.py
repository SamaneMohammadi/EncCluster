...
class Client(fl.client.NumPyClient):
	...
	def get_parameters(self, config={}):
		if not config['cluster_rnd']: # No clustering
			return [p.cpu().detach().numpy() for _,p in self.model.get_params()]
		else: # Clustering
			return weight_clustering(weights=np.concatenate([p.cpu().detach().numpy().reshape(-1, 1) \
				for _,p in self.model.get_params()]), num_clusters=config["num_clusters"])
	...
	def fit(self, parameters, config):
		...
		metrics['cid'] = self.cid # For printing purposes
