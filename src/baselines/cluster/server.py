...
from utils import reconstruct_weights
...
def aggregate_clustered_weights(results, shapes):

	#########################################################################################
	centers, indexes, num_examples, cids = zip(*[(result[0][0], result[0][1], result[1], result[2]) for result in results])
	num_examples_total = sum(num_examples)
	num_parameters = sum(np.prod(s) for s in shapes)
	# Convert to dict to maintain order!
	centers = dict(zip(cids, centers))
	num_examples = dict(zip(cids, num_examples))
	indexes = dict(zip(cids, indexes))
	#########################################################################################

	#########################################################################################
	# Measure statistics
	for cid in cids:
		num_bits = len(indexes[cid])*8 + 32*len(centers[cid])
		log(logging.INFO,f"[Client {cid}] - Size of communicated message: {num_bits} bits (bbp:{num_bits/(32*8*num_parameters):.6f}) | Number of Clusters: {len(centers[cid])}.")
	#########################################################################################

	#########################################################################################
	# Aggregate weights
	_results = [(reconstruct_weights(centers[cid],indexes[cid], shapes=shapes),num_examples[cid]) for cid in cids]
...
class _FedAvg(fl.server.strategy.FedAvg):
	...
	def __init__(self, shapes, init_cluster_rnd, *args, **kwargs):
