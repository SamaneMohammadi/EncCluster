...
os.environ['DATA_DIR'] = os.path.join(os.path.dirname(os.path.abspath(__file__)),'datasets')
...
parser.add_argument("--num_rounds",				default=500,			type=int,		help="number of federated rounds")
...
def client_fn(cid):
	...
	return Client(int(cid), num_clients=args.num_clients, model_loader=Model, model_name=args.model,
		...
		use_cutmix=args.use_cutmix, data_name=args.dataset, skewness=args.skewness, seed=args.seed)
