...
static inline bool xor8_serialize(xor8_t* obj, uint8_t* buffer) {

  memcpy(buffer, &obj->seed, sizeof(obj->seed));
  buffer += sizeof(obj->seed);

  memcpy(buffer, &obj->blockLength, sizeof(obj->blockLength));
  buffer += sizeof(obj->blockLength);

  memcpy(buffer, obj->fingerprints, 3 * obj-> blockLength);

  return true;
}

// Deserialization function
void xor8_deserialize(uint8_t* buffer, xor8_t* obj) {

  memcpy(&obj->seed, buffer, sizeof(obj->seed));
  buffer += sizeof(obj->seed);

  memcpy(&obj->blockLength, buffer, sizeof(obj->blockLength));
  buffer += sizeof(obj->blockLength);

  // Note: You'll need to ensure that the Fingerprints pointer in obj is properly allocated
  // with the necessary memory space before copying data into it.
  memcpy(obj->fingerprints, buffer, 3 * obj-> blockLength);

}

// Serialization function
static inline bool xor16_serialize(xor16_t* obj, uint8_t* buffer) {
  ...
  memcpy(buffer, obj->fingerprints, 3 * obj->blockLength * sizeof(uint16_t));

  return true;
}

// Deserialization function
void xor16_deserialize(uint8_t* buffer, xor16_t* obj) {
  ...
  // Note: You'll need to ensure that the Fingerprints pointer in obj is properly allocated
  // with the necessary memory space before copying data into it.
  memcpy(obj->fingerprints, buffer, 3 * obj-> blockLength * sizeof(uint16_t));
}
